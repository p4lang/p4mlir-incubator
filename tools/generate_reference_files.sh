...
OUTPUT_FILENAME=$DEFAULT_OUTPUT_FILENAME # Kept for future use, but ignored now
...
while IFS= read -r -d $'\0' file && IFS= read -r -d $'\0' rel_file; do
    ((processed_count++))
    ...
    if [ -n "$GLOBAL_OUTPUT_DIR" ]; then
        ...
        if ! touch "$TARGET_REF_FILE.tmp" 2>/dev/null; then
             ...
             if ! touch "$SOURCE_DIR/.tmp_write_test" 2>/dev/null; then
                 ...
                 rm "$SOURCE_DIR/.tmp_write_test"
             fi
        else
             rm "$TARGET_REF_FILE.tmp"
        fi
...
done < <(find "$SEARCH_DIR" -type f -name "*$EXTENSION" -printf '%p\0%P\0')
