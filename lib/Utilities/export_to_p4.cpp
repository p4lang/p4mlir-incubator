...
inline std::optional<mlir::DictionaryAttr> getAnnotationsAttr(mlir::Operation *op) {
    ...
    if (mlir::Attribute attr = op->getAttr(annotationAttrName)) {
        if (auto dictAttr = attr.dyn_cast<mlir::DictionaryAttr>()) {
            // Return only if it's not empty, matching behavior of specific accessors.
...
class P4HirToP4Exporter {
 ...
 private:
    ...
    mlir::LogicalResult declareVariable(P4HIR::VariableOp varOp, ExtendedFormattedOStream &ss) {
        ...
        mlir::Operation *nextOp = varOp->getNextNode();
        P4HIR::AssignOp initAssignOp;
        if (nextOp != nullptr) {
            if (auto assignOp = mlir::dyn_cast<P4HIR::AssignOp>(nextOp)) {
                if (assignOp.getRef() == varOp.getResult()) {
                    initAssignOp = assignOp;
                }
            }
        }
        ...
        if (initAssignOp) {
            ...
            ss << " = ";
            if (failed(exportExpression(initAssignOp.getValue(), ss))) {
                return mlir::failure();
            }
            ss.semicolon();
            // Mark the assignment as handled so it's skipped later.
            hoistedAssignOps.insert(initAssignOp);
        } else {
    ...
    mlir::LogicalResult exportTopLevelFuncOp(P4HIR::FuncOp &funcOp, ExtendedFormattedOStream &ss) {
        ...
        if (funcOp.getAction() || (!funcOp.isExternal())) {
            ...
            if (!funcOp.getBody().empty()) {
                ...
                // Hoist and declare variables first.
                if (failed(hoistAndDeclareVariables(entryBlock, ss))) {
                    return mlir::failure();
                }
                // Export the remaining statements.
                if (failed(exportBlock(entryBlock, ss))) {
                    return mlir::failure();
                }
            }
            ss.closeBrace();
    ...
    mlir::LogicalResult exportParserDeclaration(P4HIR::ParserOp &parserOp,
        ...
        // Hoist parser-level local variables first.
        auto &block = parserOp.getBody().front();
        if (failed(hoistAndDeclareVariables(block, ss))) {
            return mlir::failure();
        }
        // Export the remaining statements.
        if (failed(exportBlock(block, ss))) {
            return mlir::failure();
        }
        ss.closeBrace();
        return mlir::success();
    ...
    mlir::LogicalResult exportControlDeclaration(P4HIR::ControlOp &controlOp,
        ...
        // Hoist control-level local variables first.
        auto &block = controlOp.getBody().front();
        if (failed(hoistAndDeclareVariables(block, ss))) {
            return mlir::failure();
        }
        // Export the remaining statements.
    ...
    mlir::LogicalResult exportExpression(mlir::Value value, ExtendedFormattedOStream &ss) {
        ...
        if (auto ternaryOp = mlir::dyn_cast<P4HIR::TernaryOp>(op)) {
            ...
            // We add outer parentheses for operator precedence.
            ss << "(";
    ...
    mlir::LogicalResult exportConstantAttr(mlir::Attribute &constantAttr,
        ...
        if (auto intAttr = constantAttr.dyn_cast<P4HIR::IntAttr>()) {
            ...
            if (auto aliasType = type.dyn_cast<P4HIR::AliasType>()) {
                ss << "(" << aliasType.getName() << ") ";
                type = aliasType.getAliasedType();
            }
            if (auto bitsType = type.dyn_cast<P4HIR::BitsType>()) {
            ...
            if (auto intType = type.dyn_cast<P4HIR::InfIntType>()) {
                ss << intAttr.getValue();
                return mlir::success();
            }
        ...
        if (auto enumAttr = constantAttr.dyn_cast<P4HIR::EnumFieldAttr>()) {
            ...
            if (auto serEnumType = enumAttr.getType().dyn_cast<P4HIR::SerEnumType>()) {
        ...
        if (auto aggAttr = constantAttr.dyn_cast<P4HIR::AggAttr>()) {
            ss << "{";
            auto result = Utilities::interleaveCommaWithError(
        ...
        if (auto ctorAttr = constantAttr.dyn_cast<P4HIR::CtorParamAttr>()) {
            ...
            if (extractRef.getFieldName() == P4HIR::HeaderType::validityBit) {
                ...
                if (auto constOp = rvalue.getDefiningOp<P4HIR::ConstOp>()) {
                    ...
                    if (auto validityBitAttr =
                            constOp.getValue().dyn_cast<P4HIR::ValidityBitAttr>()) {
                        // Append the correct method call.
    ...
    mlir::LogicalResult exportIf(P4HIR::IfOp &ifOp, ExtendedFormattedOStream &ss) {
        ...
        if (!elseRegion.empty() && !elseRegion.front().empty()) {
            ...
            if (elseHasOps) {
                ...
                ss.openBrace();
                hoistedAssignOps.clear();
                auto &block = ifOp.getElseRegion().front();
                // Hoist and declare variables first.
                if (failed(hoistAndDeclareVariables(block, ss))) {
                    return mlir::failure();
                }
    ...
    mlir::LogicalResult exportTableDefaultActionProperty(P4HIR::TableOp &tableOp,
        ...
        for (auto &body : defaultActionOp.getBody()) {
            for (auto &op : body) {
                // Assuming the yielded value or call is the only relevant thing.
                // Attempt to export the first non-yield op as the expression.
                // This assumes a simple structure like { p4hir.call @action(); p4hir.yield }
                for (auto result : op.getResults()) {
                    if (failed(exportExpression(result, ss))) {
                        return op.emitError()
                               << "Cannot determine default action value expression.";
                    }
                    ss.semicolon();
                    return mlir::success();
                }
    ...
    mlir::LogicalResult exportControlStatement(mlir::Operation &op, ExtendedFormattedOStream &ss) {
        // Variable declarations are handled by the caller (exportControlDeclaration) via hoisting.
        if (mlir::isa<P4HIR::VariableOp>(op)) {
            return mlir::success();
        }
    ...
    mlir::LogicalResult exportControlApply(P4HIR::ControlApplyOp &op,
        ...
        ss.openBrace();
        hoistedAssignOps.clear();
        // ApplyOp should contain a single block.
        if (!op.getBody().empty()) {
    ...
    mlir::LogicalResult exportParserStatement(mlir::Operation &op, ExtendedFormattedOStream &ss) {
        // Variable declarations are handled by the caller (exportParserDeclaration) via hoisting.
        if (mlir::isa<P4HIR::VariableOp>(op)) {
            return mlir::success();
        }
        // Handled assignments were also dealt with by the caller.
    ...
    mlir::LogicalResult exportParserState(P4HIR::ParserStateOp &parserState,
        ...
        if (!parserState.getBody().empty()) {
            mlir::Block &entryBlock = parserState.getBody().front();
            // Hoist and declare state-local variables first.
            if (failed(hoistAndDeclareVariables(entryBlock, ss))) {
                return mlir::failure();
            }
    ...
    mlir::LogicalResult exportParserStateStatement(mlir::Operation &op,
                                                   ExtendedFormattedOStream &ss) {
        // Variable declarations are handled by the caller (exportParserState) via hoisting.
        if (mlir::isa<P4HIR::VariableOp>(op)) {
            return mlir::success();
        }
        // Handled assignments were also dealt with by the caller.
        ...
        // P4 allows immediate transition to accept/reject without explicit op.
        if (mlir::isa<P4HIR::ParserAcceptOp, P4HIR::ParserRejectOp>(op)) {
            // These likely correspond to the state name itself in P4 ('transition accept;').
            // The ParserTransitionOp should handle these cases by referencing accept/reject
            // symbols. If these ops exist standalone, it might indicate an MLIR structural detail.
            // For now, assume ParserTransitionOp covers the P4 syntax.
            return mlir::success();
        }
        // TODO: Handle 'transition select(...) { ... }' (ParserSelectOp).

        // Try common statements for things like assignments, method calls within the state.
        auto commonResult = exportCommonStatement(op, ss);
        if (failed(commonResult)) return mlir::failure();
        if (commonResult.value()) return mlir::success();

        // Default error for Unsupported state statements.
    ...
    /// @brief Gets the original parameter name and exports type and name. (FuncOp specific
    /// overload).
    /// @param funcOp The parent p4hir.func operation.
    /// @param index The index of the parameter.
    /// @param ss The output stream where Type and Name are written.
    /// @return Success or failure.
    mlir::LogicalResult getOriginalParamName(P4HIR::FuncOp funcOp, size_t index,
        ...
        if (!argAttrs || index >= argAttrs.size()) {
            // Fallback to FunctionOpInterface version if arg_attrs is missing or index is out of
            // bounds. This might happen for extern declarations where arg_attrs is optional.
            return getOriginalParamName(
                mlir::cast<mlir::FunctionOpInterface>(funcOp.getOperation()), index, ss);
        }
        auto dict = argAttrs[index].dyn_cast_or_null<mlir::DictionaryAttr>();
        if (!dict) {
            // Fallback if attribute isn't a dictionary.
            return getOriginalParamName(
                mlir::cast<mlir::FunctionOpInterface>(funcOp.getOperation()), index, ss);
        }
        auto nameAttr = dict.get(paramNameAttrName).dyn_cast_or_null<mlir::StringAttr>();
        if (!nameAttr || nameAttr.getValue().empty()) {
            // Fallback if name attribute is missing.
            return getOriginalParamName(
                mlir::cast<mlir::FunctionOpInterface>(funcOp.getOperation()), index, ss);
        }
