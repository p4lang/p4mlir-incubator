...
/// @brief Finds all unique MLIR types used anywhere within a ModuleOp.
/// @param moduleOp The MLIR module to scan.
/// @return A SetVector containing the unique types found.
...
//===----------------------------------------------------------------------===//
// P4HirToP4Exporter Class Definition
//===----------------------------------------------------------------------===//

/// @brief Handles the conversion of P4HIR MLIR dialect to P4 source code.
class P4HirToP4Exporter {
 ...
 private:
    ...
    /// @brief Safely retrieves the 'annotations' DictionaryAttr from an operation, if it exists.
    ...
    inline std::optional<mlir::DictionaryAttr> getAnnotationsAttr(mlir::Operation *op) {
        ...
        // Use llvm::TypeSwitch to efficiently check known Ops with the generated getter.
        return llvm::TypeSwitch<mlir::Operation *, std::optional<mlir::DictionaryAttr>>(op)
            // List all P4HIR operations known to have OptionalAttr<DictionaryAttr>:$annotations.
            .Case<P4HIR::ConstOp>([](auto typedOp) {
                return typedOp.getAnnotationsAttr() ? std::optional(typedOp.getAnnotationsAttr())
                                                    : std::nullopt;
            })
            .Case<P4HIR::VariableOp>([](auto typedOp) {
                return typedOp.getAnnotationsAttr() ? std::optional(typedOp.getAnnotationsAttr())
                                                    : std::nullopt;
            })
            .Case<P4HIR::ScopeOp>([](auto typedOp) {
                return typedOp.getAnnotationsAttr() ? std::optional(typedOp.getAnnotationsAttr())
                                                    : std::nullopt;
            })
            .Case<P4HIR::FuncOp>([](auto typedOp) {
                return typedOp.getAnnotationsAttr() ? std::optional(typedOp.getAnnotationsAttr())
                                                    : std::nullopt;
            })
            .Case<P4HIR::InstantiateOp>([](auto typedOp) {
                return typedOp.getAnnotationsAttr() ? std::optional(typedOp.getAnnotationsAttr())
                                                    : std::nullopt;
            })
            .Case<P4HIR::ExternOp>([](auto typedOp) {
                return typedOp.getAnnotationsAttr() ? std::optional(typedOp.getAnnotationsAttr())
                                                    : std::nullopt;
            })
            .Case<P4HIR::PackageOp>([](auto typedOp) {
                return typedOp.getAnnotationsAttr() ? std::optional(typedOp.getAnnotationsAttr())
                                                    : std::nullopt;
            })
            .Case<P4HIR::ParserOp>([](auto typedOp) {
                return typedOp.getAnnotationsAttr() ? std::optional(typedOp.getAnnotationsAttr())
                                                    : std::nullopt;
            })
            .Case<P4HIR::ControlOp>([](auto typedOp) {
                return typedOp.getAnnotationsAttr() ? std::optional(typedOp.getAnnotationsAttr())
                                                    : std::nullopt;
            })
            .Case<P4HIR::ForOp>([](auto typedOp) {
                return typedOp.getAnnotationsAttr() ? std::optional(typedOp.getAnnotationsAttr())
                                                    : std::nullopt;
            })
            .Case<P4HIR::ForInOp>([](auto typedOp) {
                return typedOp.getAnnotationsAttr() ? std::optional(typedOp.getAnnotationsAttr())
                                                    : std::nullopt;
            })
            // Assuming ParserStateOp has annotations.
            .Case<P4HIR::ParserStateOp>([](auto typedOp) {
                return typedOp.getAnnotationsAttr() ? std::optional(typedOp.getAnnotationsAttr())
                                                    : std::nullopt;
            })
            // Assuming TableOp has annotations.
            .Case<P4HIR::TableOp>([](auto typedOp) {
                return typedOp.getAnnotationsAttr() ? std::optional(typedOp.getAnnotationsAttr())
                                                    : std::nullopt;
            })
            // Add other specific P4HIR ops here as needed (e.g., TableOp, ParserStateOp if they
            // have it).

            // Default case: Use generic attribute checking for any other operation type.
            .Default([](mlir::Operation *op) -> std::optional<mlir::DictionaryAttr> {
                ...
                if (mlir::Attribute attr = op->getAttr(annotationAttrName)) {
                    if (auto dictAttr = attr.dyn_cast<mlir::DictionaryAttr>()) {
                        // Found it and it's a DictionaryAttr.
                        return dictAttr;
                    }
                }
                // Attribute not found.
    ...
    mlir::LogicalResult exportAnnotations(mlir::DictionaryAttr annotations,
        ...
        for (mlir::NamedAttribute namedAttr : annotations) {
            ...
            } else if (value.isa<mlir::ArrayAttr>() || value.isa<mlir::DictionaryAttr>()) {
                // It's a list or key-value map, e.g., @list(...) or @kv(...).
                ss << "(";
                // Recursively handle list/dict.
    ...
    /// @brief Declares top-level P4 constructs derived from an MLIR type if necessary.
    ///        This primarily handles struct, header, and union definitions.
    /// @param type The MLIR type to potentially declare.
    /// @param ss The output stream.
    /// @return Success or failure.
    mlir::LogicalResult declareTopLevelTypes(mlir::Type &type, ExtendedFormattedOStream &ss) {
        ...
        if (llvm::isa<P4HIR::HeaderType, P4HIR::HeaderUnionType, P4HIR::EnumType, P4HIR::ErrorType>(
                type)) {
            if (failed(declareType(type, ss))) {
                return mlir::failure();
            }
            return mlir::success();
        }
        ...
        if (auto parserType = llvm::dyn_cast<P4HIR::ParserType>(type)) {
            // The definition itself handles the declaration.
            return mlir::success();
        }
        ...
        if (mlir::isa<P4HIR::BitsType, P4HIR::BoolType, P4HIR::InfIntType, P4HIR::ValidBitType>(
                type)) {
            // Base types don't need top-level declarations.
            return mlir::success();
        }
    ...
    mlir::LogicalResult exportTopLevelStatement(mlir::Operation &op, ExtendedFormattedOStream &ss) {
        ...
        if (auto assignOp = mlir::dyn_cast<P4HIR::AssignOp>(op)) {
            // Top-level assignments are usually initializers handled elsewhere or errors.
            return mlir::success();
        }
        if (auto varOp = mlir::dyn_cast<P4HIR::VariableOp>(op)) {
            // Top-level variables are usually errors in P4.
            // If intended as global, needs checking. For now, assume it's declared.
        ...
        // Allow emitting a warning for unhandled ops but return success.
        op.emitWarning() << "Unhandled top-level operation: " << op;
        return mlir::success();
    }
    ...
    mlir::LogicalResult exportExternStatementOp(mlir::Operation &op, ExtendedFormattedOStream &ss) {
        ...
        // Allow emitting a warning for unhandled ops but return success.
        op.emitWarning() << "Unhandled operation within extern: " << op;
        return mlir::success();
    }
    ...
    mlir::LogicalResult exportInstantiateOP(P4HIR::InstantiateOp instantiateOp,
        ...
        // If instantiating a Parser, Control, or Extern directly at top level (outside package).
        // We might need to declare the instance variable.
        // This depends on whether the mainPackageOnly option implies these are handled elsewhere.
        // For now, assume these definitions are exported if needed, but the instance declaration
        // might be skipped if mainPackageOnly is true, anticipating the main package uses them.
        if (llvm::isa<P4HIR::ParserOp, P4HIR::ControlOp, P4HIR::ExternOp>(definition)) {
            if (!options().mainPackageOnly) {
    ...
    mlir::LogicalResult exportP4Type(mlir::Type type, ExtendedFormattedOStream &ss) {
        return llvm::TypeSwitch<mlir::Type, mlir::LogicalResult>(type)
            ...
            .Case<P4HIR::VoidType>([&](P4HIR::VoidType /*voidType*/) {
            ...
            // Special internal types - should not be directly exported usually.
            .Case<P4HIR::ValidBitType>([&](P4HIR::ValidBitType) {
                return mlir::emitError(mlir::UnknownLoc::get(type.getContext()))
                       << "ValidBitType cannot be directly exported as a P4 type name.";
            })
            // Array types (usually header stacks).
            .Case<P4HIR::ArrayType>([&](P4HIR::ArrayType arrayType) {
                // P4 standard arrays are usually header stacks.
            ...
            .Case<P4HIR::InfIntType>([&](P4HIR::InfIntType /*infIntType*/) {
    ...
    mlir::LogicalResult declareType(mlir::Type &type, ExtendedFormattedOStream &ss) {
        return llvm::TypeSwitch<mlir::Type, mlir::LogicalResult>(type)
            .Case<P4HIR::HeaderType>([&](auto headerType) {
            ...
            .Case<P4HIR::StructType>([&](auto structType) {
            ...
            .Case<P4HIR::HeaderUnionType>([&](auto unionType) {
            ...
            .Case<P4HIR::ErrorType>([&](auto errorType) {
                ss << P4HIR::ErrorType::getMnemonic() << " ";
                ss.openBrace();
                for (auto field : errorType.getFields()) {
                    if (failed(exportConstantAttr(field, ss))) {
                        return mlir::failure();
                    }
                    ss << ",";
                    ss.newline();
    ...
    mlir::LogicalResult exportTypeAttributes(mlir::ArrayRef<mlir::Attribute> attrs,
        ...
        auto result = Utilities::interleaveCommaWithError(attrs, ss, [&](mlir::Attribute attr) {
            if (auto typeAttr = attr.dyn_cast<mlir::TypeAttr>()) {
                return exportP4Type(typeAttr.getValue(), ss);
            }
            // TODO: Handle other attribute kinds if needed (e.g., IntAttr for sizes).
    ...
    /// @brief Attempts to detect if a scope represents a simple call pattern
    ///        (potentially with inout/out handling) and export the underlying call directly.
    /// @param scopeOp The p4hir.scope operation.
    /// @param ss The output stream.
    /// @return mlir::success if the pattern was detected and exported,
    ///         mlir::failure if the pattern doesn't match (caller should handle normally).
    /// TODO: How to handle method calls on extern members?.
    mlir::LogicalResult exportCopyInCopyOutMethodCall(P4HIR::ScopeOp &scopeOp,
                                                      ExtendedFormattedOStream &ss) {
        if (scopeOp.getNumResults() > 0) return mlir::failure();
        auto &region = scopeOp.getScopeRegion();
        if (region.empty() || !region.hasOneBlock()) return mlir::failure();
        mlir::Block &block = region.front();
        if (!mlir::isa_and_nonnull<P4HIR::YieldOp>(block.getTerminator()) ||
            block.getTerminator()->getNumOperands() > 0) {
            return mlir::failure();
        }

        // --- Find the single call operation (same as before). ---
        mlir::Operation *callOp = nullptr;
        for (mlir::Operation &op : block) {
            if (mlir::isa<P4HIR::CallOp, P4HIR::CallMethodOp>(op)) {
                if (callOp != nullptr) return mlir::failure();
                callOp = &op;
            } else if (!mlir::isa<P4HIR::VariableOp, P4HIR::ReadOp, P4HIR::AssignOp,
                                  P4HIR::StructExtractRefOp, P4HIR::YieldOp>(op)) {
                return mlir::failure();
            }
        }
        if (callOp == nullptr) return mlir::failure();

        // --- Reconstruct original arguments and export. ---
        llvm::SmallVector<mlir::Value> originalArgs;
        mlir::CallOpInterface callInterface = mlir::cast<mlir::CallOpInterface>(callOp);
        // For CallMethodOp, the *real* arguments start after the base object.
        // For CallOp, they are all arguments.
        bool isMethodCall = mlir::isa<P4HIR::CallMethodOp>(callOp);
        mlir::ValueRange mlirCallArgs = callInterface.getArgOperands();
        mlir::ValueRange logicalCallArgs = isMethodCall ? mlirCallArgs.drop_front() : mlirCallArgs;

        // Track mappings: Temp Var -> Original Source/Destination Value.
        llvm::DenseMap<mlir::Value, mlir::Value> tempVarToOriginal;

        for (mlir::Operation &op : block) {
            // Look for assignments *to* temporary variables *before* the call (Handles INOUT case).
            if (op.isBeforeInBlock(callOp)) {
                if (auto assignOp = mlir::dyn_cast<P4HIR::AssignOp>(op)) {
                    if (auto varOp = assignOp.getRef().getDefiningOp<P4HIR::VariableOp>()) {
                        // Temp var defined in scope.
                        if (varOp->getParentOp() == scopeOp) {
                            mlir::Value sourceValue = assignOp.getValue();
                            if (auto readOp = sourceValue.getDefiningOp<P4HIR::ReadOp>()) {
                                // Map temp -> original IN arg.
                                tempVarToOriginal[assignOp.getRef()] = readOp.getRef();
                            } else {
                                // Direct value/const.
                                tempVarToOriginal[assignOp.getRef()] = sourceValue;
                            }
                        }
                    }
                }
            }
            // Look for assignments *from* temporary variables *after* the call (Handles OUT case).
            else if (callOp->isBeforeInBlock(&op)) {
                if (auto assignOp = mlir::dyn_cast<P4HIR::AssignOp>(op)) {
                    if (auto readOp = assignOp.getValue().getDefiningOp<P4HIR::ReadOp>()) {
                        if (auto varOp = readOp.getRef().getDefiningOp<P4HIR::VariableOp>()) {
                            // Reading from temp var.
                            if (varOp->getParentOp() == scopeOp) {
                                // The destination of this assign is the original OUT arg.
                                // Map temp -> original OUT arg destination.
                                tempVarToOriginal[readOp.getRef()] = assignOp.getRef();
                            }
                        }
                    }
                }
            }
            // Special Case: ExtractRef before call for OUT destination.
            // Sometimes the extract_ref happens before the call itself.
            else if (op.isBeforeInBlock(callOp)) {
                if (auto extractRefOp = mlir::dyn_cast<P4HIR::StructExtractRefOp>(op)) {
                    // Look for an assignment *after* the call that uses this extractRefOp as
                    // destination.
                    for (mlir::Operation &postOp :
                         llvm::make_range(callOp->getNextNode(), block.getTerminator())) {
                        if (auto postAssign = mlir::dyn_cast<P4HIR::AssignOp>(postOp)) {
                            if (postAssign.getRef() == extractRefOp.getResult()) {
                                // Found the final assignment. Now find the temp var it reads from.
                                if (auto postRead =
                                        postAssign.getValue().getDefiningOp<P4HIR::ReadOp>()) {
                                    if (auto tempVar =
                                            postRead.getRef().getDefiningOp<P4HIR::VariableOp>()) {
                                        if (tempVar->getParentOp() == scopeOp) {
                                            // Map temp -> extract_ref result.
                                            tempVarToOriginal[postRead.getRef()] =
                                                extractRefOp.getResult();
                                            // Found the link.
                                            break;
                                        }
                                    }
        ...
        // Rebuild the argument list for the P4 call.
        // Use logicalCallArgs here.
        for (mlir::Value calledArg : logicalCallArgs) {
            if (tempVarToOriginal.count(calledArg)) {
                // If the argument passed to the call was a temp variable,.
                // use the original source/destination we tracked instead.
                originalArgs.push_back(tempVarToOriginal[calledArg]);
            } else {
                // Otherwise (e.g., for 'in' params passed directly), use the argument.
                originalArgs.push_back(calledArg);
            }
        }

        // --- Export the simplified call. ---
        mlir::LogicalResult status = mlir::failure();
        // Export annotations from the original scope.
        if (auto annots = getAnnotationsAttr(scopeOp)) {
            if (failed(exportAnnotations(*annots, ss))) return mlir::failure();
        }

        if (auto directCall = mlir::dyn_cast<P4HIR::CallOp>(callOp)) {
            ss << directCall.getCalleeAttr().getLeafReference().getValue() << "(";
            status = Utilities::interleaveCommaWithError(
                originalArgs, ss, [&](mlir::Value arg) { return exportExpression(arg, ss); });
            ss << ")";

        } else if (auto methodCall = mlir::dyn_cast<P4HIR::CallMethodOp>(callOp)) {
            // Base is the first operand of the *MLIR* call.
            mlir::Value baseObject = mlirCallArgs[0];

            if (failed(exportExpression(baseObject, ss))) return mlir::failure();
            ss << "." << methodCall.getCallee().getLeafReference().getValue();
            if (auto typeOperands = methodCall.getTypeOperandsAttr()) {
                if (failed(exportTypeAttributes(typeOperands.getValue(), ss)))
                    return mlir::failure();
            }
            ss << "(";
            // Use the reconstructed 'originalArgs' which correspond to logicalCallArgs.
            status = Utilities::interleaveCommaWithError(
                originalArgs, ss, [&](mlir::Value arg) { return exportExpression(arg, ss); });
            ss << ")";
        }
    ...
    mlir::LogicalResult exportExpression(mlir::Value value, ExtendedFormattedOStream &ss) {
        ...
        mlir::Location loc =
            (op != nullptr) ? op->getLoc() : mlir::UnknownLoc::get(value.getContext());
        ...
        if (auto castOp = mlir::dyn_cast<P4HIR::CastOp>(op)) {
            ...
            ss << ")(";
            // Parenthesize source for safety.
        ...
        if (auto structExtractOp = mlir::dyn_cast<P4HIR::StructExtractOp>(op)) {
            // P4 syntax: struct_instance.field_name or header_instance.isValid().
        ...
        if (auto ternaryOp = mlir::dyn_cast<P4HIR::TernaryOp>(op)) {
            ...
            // We add outer parentheses for operator precedence safety.
            ss << "(";
            ...
            mlir::Region &trueRegion = ternaryOp.getTrueRegion();
            ...
            auto trueYield = mlir::dyn_cast_or_null<P4HIR::YieldOp>(trueBlock.getTerminator());
            if (!trueYield) {
                return ternaryOp.emitError()
            ...
            // Parenthesize the sub-expression for safety.
            ss << "(";
            ...
            mlir::Region &falseRegion = ternaryOp.getFalseRegion();
            ...
            auto falseYield = mlir::dyn_cast_or_null<P4HIR::YieldOp>(falseBlock.getTerminator());
            if (!falseYield) {
                return ternaryOp.emitError()
        ...
        if (auto uninitOp = mlir::dyn_cast<P4HIR::UninitializedOp>(op)) {
            // P4 has no standard uninitialized value. Needs lowering or default value.
            return uninitOp.emitError() << "p4hir.uninitialized has no direct P4 equivalent.";
        }
        if (auto structOp = mlir::dyn_cast<P4HIR::StructOp>(op)) {
            // Struct constructor expression: StructType { field1 = val1, ... }.
            if (auto structType = structOp.getResult().getType().dyn_cast<P4HIR::StructType>()) {
                ss << structType.getName() << " {";
            } else if (auto headerType =
                           structOp.getResult().getType().dyn_cast<P4HIR::HeaderType>()) {
                ss << headerType.getName() << " {";
            } else {
                return structOp.emitError()
                       << "Unsupported type in StructOp: " << structOp.getResult().getType();
            }
            auto iface = structOp.getResult().getType().dyn_cast<P4HIR::StructLikeTypeInterface>();
            ...
            for (size_t idx = 0; idx < fields.size(); ++idx) {
                if (fields[idx].type.isa<P4HIR::ValidBitType>()) {
                    continue;
                }
                if (idx > 0) {
                    ss << ", ";
        ...
        if (auto arrayOp = mlir::dyn_cast<P4HIR::ArrayOp>(op)) {
            // Array constructor expression (like {elem1, elem2}) doesn't have direct P4.
            // equivalent.
            return arrayOp.emitError()
                   << "p4hir.array construction has no direct P4 expression equivalent.";
        }

        return op->emitError() << "Unhandled operation in exportExpression: " << *op;
    }

    /// @brief Exports a P4 constant value from a p4hir.const operation's attribute.
    /// @param constant The ConstOp containing the constant value.
    /// @param ss The output stream.
    /// @return Success or failure.
    /// @example P4 Code.
    /// ```p4
    ...
    /// "hello" // from "hello".
    /// ```
    static mlir::LogicalResult exportConstantAttr(mlir::Attribute &constantAttr,
                                                  ExtendedFormattedOStream &ss) {
        mlir::Location loc = mlir::UnknownLoc::get(constantAttr.getContext());
        if (auto intAttr = constantAttr.dyn_cast<P4HIR::IntAttr>()) {
            if (auto bitsType = intAttr.getType().dyn_cast<P4HIR::BitsType>()) {
                unsigned width = bitsType.getWidth();
                const llvm::APInt &value = intAttr.getValue();
                // Emit width and suffix (s for signed, w for unsigned/bit).
                ss << width << (bitsType.isSigned() ? "s" : "w");
                // Emit the value in decimal.
                std::string decimalString;
                llvm::raw_string_ostream decimalStream(decimalString);
                // Print unsigned representation for positive/zero, let P4 handle interpretation.
                value.print(decimalStream, /*isSigned=*/false);
                ss << decimalStream.str();
                return mlir::success();
            }
            if (auto intType = intAttr.getType().dyn_cast<P4HIR::InfIntType>()) {
                ss << intAttr.getValue();
                return mlir::success();
            }
            return mlir::emitError(loc) << "Unsupported type in IntAttr: " << intAttr.getType();
        }
        if (auto strAttr = constantAttr.dyn_cast<mlir::StringAttr>()) {
            ss << escapeP4String(strAttr.getValue());
            return mlir::success();
        }
        if (auto boolAttr = constantAttr.dyn_cast<P4HIR::BoolAttr>()) {
            ss << (boolAttr.getValue() ? "true" : "false");
            return mlir::success();
        }
        // TODO: Handle EnumFieldAttr, ErrorCodeAttr, AggAttr, MatchKindAttr etc..
        if (auto enumAttr = constantAttr.dyn_cast<P4HIR::EnumFieldAttr>()) {
            // P4 uses dot notation: EnumTypeName.FieldName.
            if (auto enumType = enumAttr.getType().dyn_cast<P4HIR::EnumType>()) {
                ss << enumType.getName() << "." << enumAttr.getField();
                return mlir::success();
            }
            // Handle SerEnumType similarly if needed.
            return mlir::emitError(loc)
                   << "EnumFieldAttr requires EnumType, found " << enumAttr.getType();
        }
        if (auto errorAttr = constantAttr.dyn_cast<P4HIR::ErrorCodeAttr>()) {
            // P4 error constants: error.ErrorName.
            ss << "error." << errorAttr.getField();
            return mlir::success();
        }
        if (auto strAttr = constantAttr.dyn_cast<mlir::StringAttr>()) {
            ss << escapeP4String(strAttr.getValue());
        ...
        if (auto intAttr = constantAttr.dyn_cast<mlir::IntegerAttr>()) {
            // Export based on the underlying MLIR integer type (e.g., i64).
            ss << intAttr.getValue();
            return mlir::success();
        }
        if (auto symRefAttr = constantAttr.dyn_cast<mlir::SymbolRefAttr>()) {
            // Export the leaf name for symbol references within annotations.
            ss << symRefAttr.getLeafReference().getValue();
            return mlir::success();
        }
        if (auto matchKindAttr = constantAttr.dyn_cast<P4HIR::MatchKindAttr>()) {
            // Match kinds are typically identifiers in P4.
            ss << matchKindAttr.getValue().getValue();
            return mlir::success();
        }
        // Add other P4HIR specific attributes if they can be annotation values (e.g., EnumField?).
        // Handle recursive structures: Arrays and Dictionaries.
        if (auto arrayAttr = constantAttr.dyn_cast<mlir::ArrayAttr>()) {
            // P4 doesn't have a direct list literal syntax *inside* annotations like @name({a,b}).
            // Common P4 annotations like @pragma often take multiple space-separated strings.
            // Or it might be @name(a, b). Let's assume the latter for now.
            // We export the elements directly, comma-separated. Caller adds ().
            return Utilities::interleaveCommaWithError(
                arrayAttr.getValue(), ss,
                [&](mlir::Attribute elem) { return exportConstantAttr(elem, ss); });
        }
        if (auto dictAttr = constantAttr.dyn_cast<mlir::DictionaryAttr>()) {
            // P4 doesn't have a direct dict literal syntax *inside* annotations like @name({k=v}).
            // The closest is @name(k=v, k2=v2). Let's export in that format.
            // Caller adds ().
            return Utilities::interleaveCommaWithError(
                dictAttr.getValue(), ss, [&](mlir::NamedAttribute namedAttr) {
                    ss << namedAttr.getName().getValue() << " = ";
                    auto valueAttr = namedAttr.getValue();
                    return exportConstantAttr(valueAttr, ss);
                });
        }
        if (auto aggAttr = constantAttr.dyn_cast<P4HIR::CtorParamAttr>()) {
            // TODO: Where do we need this?.
            return mlir::success();
        }
        if (auto validityAttr = constantAttr.dyn_cast<P4HIR::ValidityBitAttr>()) {
            // The validity bit is implicit in P4.
            return mlir::success();
        }
        return mlir::emitError(loc)
    ...
    mlir::LogicalResult exportLValue(mlir::Value value, ExtendedFormattedOStream &ss) {
        ...
        if (auto extractRefOp = mlir::dyn_cast<P4HIR::StructExtractRefOp>(op)) {
            ...
            // Handle header validity bit assignment via setValid/setInvalid.
            if (extractRefOp.getFieldName() == P4HIR::HeaderType::validityBit) {
                // The assignment itself needs to handle setValid/setInvalid.
                // For forming the L-Value string, we stop at the base header.
                // The AssignOp handler will check if the target LValue's defining op.
                // is a StructExtractRefOp targeting the validity bit.
                return mlir::success();
            }
            // Normal field access.
        ...
        return op->emitError() << "Unsupported operation defining LValue: " << *op;
    }
    ...
    static mlir::LogicalResult exportParameterReference(mlir::Value value,
        ...
        if (auto blockArg = value.dyn_cast<mlir::BlockArgument>()) {
            ...
            if (auto fun = mlir::dyn_cast<mlir::FunctionOpInterface>(parentOp)) {
                ...
                // If name attribute is missing, maybe fallback to MLIR argument name?.
                // This might happen for generated blocks. Standard params should have names.
                return fun->emitError()
                       << "Missing param name attribute for function arg " << argIndex;
            }
            // Handle other block argument contexts if necessary (e.g., loop variables).
            // For now, assume function-like parameters.
    ...
    static mlir::FailureOr<llvm::StringRef> getUnaryOpString(P4HIR::UnaryOpKind kind) {
        ...
        // Should be unreachable if kind is valid.
        return mlir::failure();
    }
    ...
    mlir::LogicalResult exportP4Declaration(P4HIR::VariableOp &varOp,
        ...
        auto refType = varOp.getResult().getType().dyn_cast<P4HIR::ReferenceType>();
        if (!refType) {
        ...
        if (auto arrayType = objectType.dyn_cast<P4HIR::ArrayType>()) {
            if (failed(exportP4Type(arrayType.getElementType(), ss))) {
                return mlir::failure();
            }
            ss << " ";
        ...
        // Semicolon is usually added by the caller context (e.g., exportControlApplyStatement).
        return mlir::success();
    }
        ...
        if (auto extractRef = lvalue.getDefiningOp<P4HIR::StructExtractRefOp>()) {
            if (extractRef.getFieldName() == P4HIR::HeaderType::validityBit) {
                ...
                if (auto constOp = rvalue.getDefiningOp<P4HIR::ConstOp>()) {
                    if (auto boolAttr = constOp.getValue().dyn_cast<P4HIR::BoolAttr>()) {
                ...
                // Handle assigning a non-constant boolean expression if needed.
                return assignOp.emitError() << "Assigning dynamic expression to header validity "
                                               "bit needs specific handling or lowering.";
            }
        }

        // Default assignment case: lvalue = rvalue;.
    ...
    /// @brief Exports a combined variable declaration and initialization statement.
    ///        This is used when a VariableOp is immediately followed by an AssignOp.
    /// @param assignOp The AssignOp providing the initial value. The LValue must be defined by a
    /// VariableOp.
    /// @param ss The output stream.
    /// @return Success or failure.
    /// @example P4 Code.
    /// ```p4
    /// bit<32> myVar = initialValue;
    /// ```
    mlir::LogicalResult exportAssigningDeclaration(P4HIR::AssignOp assignOp,
                                                   ExtendedFormattedOStream &ss) {
        // Get the VariableOp being initialized.
        auto varOp = assignOp.getRef().getDefiningOp<P4HIR::VariableOp>();
        if (!varOp) {
            return assignOp.emitError()
                   << "AssignOp in declaration context does not have a VariableOp LValue.";
        }

        // Export the declaration part (Type Name).
        if (failed(exportP4Declaration(varOp, ss))) {
            return mlir::failure();
        }

        // Export the initializer part (= Value).
        ss << " = ";
        mlir::Value value = assignOp.getValue();
        if (failed(exportExpression(value, ss))) {
            return mlir::failure();
        }
        ss.semicolon();
        return mlir::success();
    ...
    mlir::LogicalResult exportIf(P4HIR::IfOp &ifOp, ExtendedFormattedOStream &ss) {
        ...
        if (!ifOp.getThenRegion().empty()) {
            // Assume simple regions with one block for typical if/else.
        ...
        if (!elseRegion.empty() && !elseRegion.front().empty()) {
            // Check if the else block is non-empty before printing "else".
            // (MLIR might create an empty block even if no 'else' was in source).
            bool elseHasOps = !elseRegion.front().getOperations().empty() ||
                              ((elseRegion.front().getTerminator() != nullptr) &&
                               !mlir::isa<P4HIR::YieldOp>(elseRegion.front().getTerminator()));
            // Heuristic: Check if terminator is YieldOp (often implicit end).
            if ((elseRegion.front().getTerminator() != nullptr) &&
                mlir::isa<P4HIR::YieldOp>(elseRegion.front().getTerminator()) &&
                elseRegion.front().getOperations().size() == 1) {
                // Treat yield-only block as empty for 'else' emission.
                elseHasOps = false;
            }
            ...
            if (elseHasOps) {
                ss << " else ";
                ss.openBrace();
                if (failed(exportBlock(elseRegion.front(), ss))) {
                    return mlir::failure();
                }
                ss.closeBrace();
            }
    ...
    //===------------------------------------------------------------------===//
    // Statement Exporting (Block & Context Dispatch).
    //===------------------------------------------------------------------===//

    /// @brief Exports the operations within a single MLIR block based on the parent context.
    ...
    mlir::LogicalResult exportBlock(mlir::Block &block, ExtendedFormattedOStream &ss) {
        for (mlir::Operation &op : block.getOperations()) {
            ...
            mlir::LogicalResult status = mlir::failure();  // Default to failure.

            // Determine context and dispatch to the appropriate statement exporter.
            if (mlir::isa<P4HIR::ControlApplyOp, P4HIR::FuncOp, P4HIR::IfOp, P4HIR::ScopeOp>(
                    parentOp)) {
                // Contexts typically found in control logic, actions, functions.
                if (mlir::isa<P4HIR::YieldOp>(op) && &op == block.getTerminator()) {
                    // Non-value-returning yields at the end of blocks are usually implicit P4.
                    // block ends.
                    status = mlir::success();
                } else if (mlir::isa<P4HIR::ReturnOp>(op) && &op == block.getTerminator()) {
                    // Return *is* exported.
                    status = exportControlApplyStatement(op, ss);
                } else {
                    // Handle regular statements within these control-like contexts.
                    status = exportControlApplyStatement(op, ss);
                }
            } else if (mlir::isa<P4HIR::ParserStateOp>(parentOp)) {
                // Context is within a parser state.
                if (mlir::isa<P4HIR::ParserTransitionOp, P4HIR::ParserAcceptOp,
                              P4HIR::ParserRejectOp>(op) &&
                    &op == block.getTerminator()) {
                    // Transitions at the end of a state block are handled by state export.
                    status = mlir::success();
                } else {
                    status = exportParserStateStatement(op, ss);
                }
            } else if (mlir::isa<P4HIR::ControlOp>(parentOp)) {
                // Context is directly within a control block (locals, actions, tables).
                status = exportControlStatement(op, ss);
            } else if (mlir::isa<P4HIR::ParserOp>(parentOp)) {
                // Context is directly within a parser block (locals, states).
                status = exportParserStatement(op, ss);
            } else {
                status = op.emitError() << "Unhandled parent context (" << parentOp->getName()
                                        << ") in exportBlock for op: " << op;
            }
    ...
    //===------------------------------------------------------------------===//
    // Statement Exporting (Control/Action Context).
    //===------------------------------------------------------------------===//

    /// @brief Exports a statement found within a control's apply block or an action's body.
    /// @param op The MLIR operation to export.
    /// @param ss The output stream.
    /// @return Success or failure.
    mlir::LogicalResult exportControlApplyStatement(mlir::Operation &op,
                                                    ExtendedFormattedOStream &ss) {
        // Operations that primarily define values used in expressions are often skipped.
        ...
        if (mlir::isa<P4HIR::ReadOp, P4HIR::CastOp, P4HIR::StructExtractOp,
                      ...
                      P4HIR::ArrayGetOp, P4HIR::UninitializedOp>(op)) {
        ...
        if (auto assignOp = mlir::dyn_cast<P4HIR::AssignOp>(op)) {
            // TODO: Check if this assignment is initializing a variable declared just before.
            if (auto varOp = assignOp.getRef().getDefiningOp<P4HIR::VariableOp>()) {
                // Check if the VariableOp was the *immediately* preceding operation.
                // This check is fragile; a better way involves tracking initializations.
                // For now, assume separate declaration and assignment.
            }
            // Default: export as a standalone assignment.
            return exportAssignmentStatement(assignOp, ss);
        }
        if (auto assignSliceOp = mlir::dyn_cast<P4HIR::AssignSliceOp>(op)) {
            return exportAssignSliceStatement(assignSliceOp, ss);
        }
        if (auto varOp = mlir::dyn_cast<P4HIR::VariableOp>(op)) {
            // Check if this variable has an immediate initializer (AssignOp following it).
            mlir::Operation *nextOp = op.getNextNode();
            if (nextOp != nullptr) {
                if (auto assignInit = mlir::dyn_cast<P4HIR::AssignOp>(nextOp)) {
                    if (assignInit.getRef() == varOp.getResult()) {
                        // Found an immediate assignment, export as combined declaration.
                        if (failed(exportAssigningDeclaration(assignInit, ss))) {
                            return mlir::failure();
                        }
                    }
                }
            }
            // Default: export as a declaration statement.
            // if (failed(exportP4Declaration(varOp, ss))) {
            //     return mlir::failure();
            // }
            // ss.semicolon(); // Semicolon part of the P4 syntax being commented out.
            return mlir::success();
        }
        if (auto returnOp = mlir::dyn_cast<P4HIR::ReturnOp>(op)) {
            ...
            // If 0 operands, just "return".
            ss.semicolon();
            return mlir::success();
        }
        if (auto ifOp = mlir::dyn_cast<P4HIR::IfOp>(op)) {
            return exportIf(ifOp, ss);
        }
        if (auto callOp = mlir::dyn_cast<P4HIR::CallOp>(op)) {
        ...
        if (auto scopeOp = mlir::dyn_cast<P4HIR::ScopeOp>(op)) {
            // P4 doesn't have explicit general scopes like C++'s {}.
            // Emit the statements within the scope directly.
            if (scopeOp.getNumResults() > 0) {
                return scopeOp.emitError()
                       << "Value-returning p4hir.scope export not yet implemented.";
            }
            if (!scopeOp.getScopeRegion().empty()) {
                if (failed(exportBlock(scopeOp.getScopeRegion().front(), ss))) {
                    return mlir::failure();
                }
            }
            return mlir::success();
        ...
        // TODO: Add SwitchOp, ForOp, ForInOp, TableApplyOp etc.

        // Default case for unhandled operations.
        op.emitWarning() << " Unhandled control/apply statement: " << op;
        // Return success to allow processing rest of block.
        return mlir::success();
    }
    ...
    mlir::LogicalResult exportTableDeclaration(P4HIR::TableOp &tableOp,
        ...
        // --- Export Table Components By Finding Ops. ---
        // The order matters for P4 syntax (key, actions, then others).
        ...
        // TODO: Export Const Entries if represented by a specific op.
        ss.closeBrace();
        return mlir::success();
    }
        ...
        for (auto &entryOp : actionsOp.getBody().front()) {
            if (auto actionEntryOp = mlir::dyn_cast<P4HIR::TableActionOp>(entryOp)) {
                ...
                for (auto &block : callBody) {
                    for (auto &op : block) {
                        if (auto scopeOp = mlir::dyn_cast<P4HIR::ScopeOp>(op)) {
                            if (failed(exportCopyInCopyOutMethodCall(scopeOp, ss))) {
                                return mlir::failure();
                            }
                            printedCall = true;
                            break;
    ...
    mlir::LogicalResult exportTableDefaultActionProperty(P4HIR::TableOp &tableOp,
        ...
        // Get first op in region.
        mlir::Operation *valueOp = &defaultActionOp.getBody().front().front();

        // Assume the first op in the region is the action call itself (e.g., CallOp).
        // Need to handle this based on what `visit(expr)` generates inside the region.
        // Let's assume exportExpression can handle the relevant op (e.g., CallOp).
        // Assuming single result.
        mlir::Value defaultValue = valueOp->getResult(0);
        if (failed(exportExpression(defaultValue, ss))) {
            return mlir::failure();
        }
        ...
        for (auto &op : tableOp.getBody().front()) {
            if (auto entryOp = mlir::dyn_cast<P4HIR::TableEntryOp>(op)) {
                ...
                if (entryOp.getBody().empty() || entryOp.getBody().front().empty() ||
                    (entryOp.getBody().front().getTerminator() == nullptr) ||
                    !mlir::isa<P4HIR::YieldOp>(entryOp.getBody().front().getTerminator())) {
                    return entryOp.emitError() << "TableEntryOp region must yield a value.";
                }
                auto yieldOp =
                    mlir::cast<P4HIR::YieldOp>(entryOp.getBody().front().getTerminator());
    ...
    mlir::LogicalResult exportControlStatement(mlir::Operation &op, ExtendedFormattedOStream &ss) {
        ...
        // Locals (Variable Declarations with potential initializers).
        if (auto varOp = mlir::dyn_cast<P4HIR::VariableOp>(op)) {
            // Check for immediate assignment for combined declaration.
            mlir::Operation *nextOp = op.getNextNode();
            if (nextOp != nullptr) {
                if (auto assignInit = mlir::dyn_cast<P4HIR::AssignOp>(nextOp)) {
                    if (assignInit.getRef() == varOp.getResult()) {
                        ...
                        // Need to skip the assignOp when the loop gets there. Fragile.
                        // Sticking to separate decl/assign is safer for now.
                        // Fall through to separate declaration:.
                    }
                }
            }
            // Default: separate declaration.
            // if (failed(exportP4Declaration(varOp, ss))) {.
            //     return mlir::failure();.
            // }.
            // ss.semicolon(); // Semicolon part of the P4 syntax being commented out.
        ...
        // Handle immediate assignments to already declared locals (should be rare here).
        if (auto assignOp = mlir::dyn_cast<P4HIR::AssignOp>(op)) {
            // If this assignOp was part of an initialization handled above, skip it.
            // This requires coordination or assuming separate decl/assign.
            // For now, assume it's a later assignment (though unusual at control scope).
            // return exportAssignmentStatement(assignOp, ss);.
            // More likely, this is an error or related to table entries etc.
            // Skip for now, assuming handled by combined decl or applies later.
            return mlir::success();
        }
        // Action Definitions.
        if (auto funcOp = mlir::dyn_cast<P4HIR::FuncOp>(op)) {
        ...
        // Other ops are likely errors or need specific handlers.
        op.emitWarning() << " Unhandled control statement: " << op;
        return mlir::success();
    }
    ...
    mlir::LogicalResult exportControlApply(P4HIR::ControlApplyOp &op,
        ...
        ss << "apply ";
        ss.openBrace();
        // ApplyOp contains a single block.
        if (!op.getBody().empty()) {
    ...
    ///        This typically includes local variables and states.
    /// @param op The MLIR operation to export.
    /// @param ss The output stream.
    /// @return Success or failure.
    mlir::LogicalResult exportParserStatement(mlir::Operation &op, ExtendedFormattedOStream &ss) {
        // Local variable declarations (often headers or temporary values).
        if (auto varOp = mlir::dyn_cast<P4HIR::VariableOp>(op)) {
            ...
            if (nextOp != nullptr) {
                if (auto assignInit = mlir::dyn_cast<P4HIR::AssignOp>(nextOp)) {
                    if (assignInit.getRef() == varOp.getResult()) {
                        ...
                        // Skip varOp, skip assignOp later. Safer: separate decl/assign.
                        // Fall through:.
                    }
                }
            }
        ...
        // Assignments (likely initializers, handled above or skipped).
        if (auto assignOp = mlir::dyn_cast<P4HIR::AssignOp>(op)) {
            // If this was handled by exportAssigningDeclaration, skip. Assume skipped for now.
            return mlir::success();
        }
        // State Definitions.
        if (auto parserState = mlir::dyn_cast<P4HIR::ParserStateOp>(op)) {
            auto parserStateName = parserState.getName();
            // P4 implicitly defines accept/reject states.
            ...
            // Export the transition statement at the end of the block.
            mlir::Operation *terminator = parserState.getBody().front().getTerminator();
            if (terminator == nullptr) {
                return parserState.emitError() << "Parser state block is not terminated.";
            }
            if (failed(exportParserStateStatement(*terminator, ss))) {
                return mlir::failure();
            }
        ...
        // Other ops are likely errors.
        op.emitWarning() << " Unhandled parser statement: " << op;
        return mlir::success();
    }
    ...
    mlir::LogicalResult exportParserStateStatement(mlir::Operation &op,
        ...
        // Value-producing ops used in expressions later - skip direct emission.
        if (mlir::isa<P4HIR::ReadOp, P4HIR::CastOp, P4HIR::StructExtractOp,
                      P4HIR::StructExtractRefOp, P4HIR::SliceOp, P4HIR::SliceRefOp, P4HIR::ConstOp,
                      P4HIR::UnaryOp, P4HIR::BinOp, P4HIR::CmpOp, P4HIR::CallOp, P4HIR::CallMethodOp
                      // Add others if needed.
                      >(op)) {
            if (op.use_empty()) {
                // op.emitWarning() << "Operation result is unused in parser state: " << op;.
            }
            return mlir::success();
        }

        // Variable Declarations (temporary state variables).
        if (auto varOp = mlir::dyn_cast<P4HIR::VariableOp>(op)) {
            // Check for immediate assignment.
            mlir::Operation *nextOp = op.getNextNode();
            if (nextOp != nullptr) {
        ...
        // Assignments.
        if (auto assignOp = mlir::dyn_cast<P4HIR::AssignOp>(op)) {
            // Assume not handled by combined decl above for now.
            return exportAssignmentStatement(assignOp, ss);
        }
        if (auto assignSliceOp = mlir::dyn_cast<P4HIR::AssignSliceOp>(op)) {
            return exportAssignSliceStatement(assignSliceOp, ss);
        }
        // Method Calls (e.g., pkt.extract()).
        if (auto callMethodOp = mlir::dyn_cast<P4HIR::CallMethodOp>(op)) {
            // Export as statement.
            if (failed(exportExpression(callMethodOp.getBase(), ss))) {
                return mlir::failure();
        ...
        // Transition Statements (handled at the end of state block export).
        ...
        if (auto scopeOp = mlir::dyn_cast<P4HIR::ScopeOp>(op)) {
            // Emit statements within the scope directly.
            if (scopeOp.getNumResults() > 0) {
                return scopeOp.emitError()
                       << "Value-returning p4hir.scope export not yet implemented.";
            }
        ...
        op.emitWarning() << " Unhandled parser state statement: " << op;
        return mlir::success();
    }
    ...
    mlir::LogicalResult exportArgument(mlir::FunctionOpInterface functionInterface,
        ...
        if (directionAttr) {
            if (auto castDir = directionAttr.dyn_cast<P4HIR::ParamDirectionAttr>()) {
    ...
    mlir::LogicalResult getOriginalParamName(mlir::FunctionOpInterface funcOp, size_t index,
                                             ExtendedFormattedOStream &ss) {
        // Get the type first.
        if (index >= funcOp.getNumArguments()) {
            return funcOp->emitError() << " Invalid argument index: " << index;
        }
        mlir::Type paramType = funcOp.getArgument(index).getType();
        if (failed(exportP4Type(paramType, ss))) {
            return mlir::failure();
        }
        ss << " ";

        // Now get the name from the attribute.
        auto nameAttr = funcOp.getArgAttr(index, P4HIR::FuncOp::getParamNameAttrName());
        if (nameAttr) {
            if (auto nameStrAttr = nameAttr.dyn_cast<mlir::StringAttr>()) {
                if (!nameStrAttr.getValue().empty()) {
                    ss << nameStrAttr.getValue();
                    return mlir::success();
                }
            }
        ...
        return funcOp->emitError()
               << "Missing or invalid param name attribute for function arg " << index;
    }

    mlir::LogicalResult getOriginalParamName(P4HIR::FuncOp funcOp, size_t index,
                                             ExtendedFormattedOStream &ss) {
